...
class SampleAVAFrames(SampleFrames):
    ...
    def __init__(self, clip_len, frame_interval=2, test_mode=False):
        #clip_len：每个采样输出片段的帧数，这是一个必需参数。
        #frame_interval=2相邻采样帧之间的时间间隔，默认为2。
        super().__init__(clip_len, frame_interval, test_mode=test_mode)

    def _get_clips(self, center_index, skip_offsets, shot_info):
        #其主要功能是根据给定的中心索引、跳过偏移量和镜头信息来计算采样帧的索引。
        """Get clip offsets."""
        #center_index：采样帧的中心索引，是计算采样帧范围的基准点。
        #skip_offsets：跳过的偏移量列表，在非测试模式下会添加到采样帧索引中。
        #shot_info：一个包含两个元素的元组，分别表示镜头的起始索引和结束索引，用于限制采样帧索引的范围。
        start = center_index - (self.clip_len // 2) * self.frame_interval    #self.clip_len：每个采样输出片段的帧数，由类的初始化参数确定
        end = center_index + ((self.clip_len + 1) // 2) * self.frame_interval  # self.frame_interval：相邻采样帧的时间间隔，同样由类的初始化参数确定。
        frame_inds = list(range(start, end, self.frame_interval))            #start：采样帧的起始索引，通过中心索引减去半个片段帧数乘以帧间隔计算得到。
        if not self.test_mode:                                               #end：采样帧的结束索引，通过中心索引加上半个片段帧数（向上取整）乘以帧间隔计算得到。
    ...
    def transform(self, results):
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                ...
                results = {
                    'fps': 30,           # 帧率
                    'timestamp': 57,     # 标注时间戳（秒）
                    'timestamp_start': 0, # 视频起始时间
                    'start_index': 0,     # 起始帧偏移通常为0
                    'shot_info': (0, 1800) # 镜头范围（帧）
                            }
        """
        fps = results['fps']
        ...
        center_index = fps * (timestamp - timestamp_start) + start_index    #根据帧率、时间戳差和起始索引计算出的采样中心帧的索引。

        skip_offsets = np.random.randint(
            -self.frame_interval // 2, (self.frame_interval + 1) // 2,
            size=self.clip_len)
        frame_inds = self._get_clips(center_index, skip_offsets, shot_info)    #生成采样窗口
...
class RawFrameDecode(BaseTransform):
    ...
    - frame_dir      ：存储视频帧的目录路径。程序会从这个目录里读取视频帧文件。
    - filename_tmpl  ：视频帧文件名的模板。例如 img_{:05}.jpg 表示文件名格式为 img_00001.jpg、img_00002.jpg 等。
    - frame_inds     ：要加载的帧索引列表。
    - modality       ：视频帧的模态，例如 RGB、Flow 等。
    - offset (optional)

    Added Keys:

    - img            ：加载并解码后的视频帧数据，通常是 NumPy 数组。
    - img_shape      ：加载后图像的形状，格式为 (height, width, channels)。
    - original_shape ：图像的原始形状，同样是 (height, width, channels) 格式。
    ...
    def transform(self, results: dict) -> dict:
        #该方法的主要功能是根据给定的索引从指定目录加载并解码视频帧，同时根据数据模态（如 RGB 或光流）进行不同处理，最后更新边界框和建议框的尺寸
        ...
        #从 results 字典中提取视频帧所在目录 directory、文件名模板 filename_tmpl 和数据模态 modality。
        if self.file_client is None:
            self.file_client = FileClient(self.io_backend, **self.kwargs)
